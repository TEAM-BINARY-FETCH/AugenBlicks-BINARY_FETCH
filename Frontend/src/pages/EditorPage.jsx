...
export default function EditorPage() {
  ...
  useEffect(() => {
    const initializeEditor = () => {
      if (!editorRef.current && document.getElementById("editorjs")) {
        ...
        const savedData = savedProjects[projectId] || { blocks: [{ type: "paragraph", data: { text: "Start typing..." } }] };
        ...
        const editorInstance = new EditorJS({
          ...
          async onChange() {
            try {
              ...
              localStorage.setItem("projects", JSON.stringify({ [projectId]: content }));
            } catch (error) {
  ...
  return (
    <div className="h-auto min-h-screen p-6 bg-gray-900 text-white">
      ...
      <div id="editorjs" className="p-4 rounded-lg w-full bg-gray-800 text-white min-h-[500px]"></div>
    </div>
  );
}
